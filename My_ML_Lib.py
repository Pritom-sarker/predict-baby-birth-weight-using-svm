...
def get_data(place,method,attr,terget):
    # Attribrute -> list of index we need (X) if we need all then type 'all'
...
def cross_validation(X,Y,model,fold,model_name):
    #IF the score is low then big chance of getting accuracy
    from sklearn.model_selection import cross_val_score

    forest_scores = cross_val_score(model, X, Y, cv=fold,
                                    scoring="neg_mean_squared_error")
    forest_rmse_scores = np.sqrt(-forest_scores)
    print('Avrage score after cross validations : {}'.format(forest_rmse_scores.mean()))
    f = open("log.txt", "a")
    currentDT = datetime.datetime.now()
    f.write('->{}  || Algo-> {} || Score -> {}\n'.format(currentDT,model_name,forest_rmse_scores.mean()))
...
def plot_matrix_(X):
    ...
    col = X.columns.values.tolist()
    scatter_matrix(X[col],figsize=(50,25))
    plt.savefig('plot_matrix.jpg')

def save_model_vocab(cv):
